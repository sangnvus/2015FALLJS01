...
namespace DDL_CapstoneProject.Controllers.ApiControllers
{
    public class UserApiController : BaseApiController
    {
        ...
        [HttpGet]
        public IHttpActionResult GetBackingForUserExport()
        {
            List<AdminBakingFullInforDTO> backingList = new List<AdminBakingFullInforDTO>();
            // Check authen.
            if (User.Identity == null || !User.Identity.IsAuthenticated)
            ...
            try
            {
                backingList = UserRepository.Instance.GetBackingForUserExport(User.Identity.Name);
            }
            catch (KeyNotFoundException)
            {
                return
        ...
        [HttpGet]
        public IHttpActionResult GetBackingForCreatedProjectExport(string projectcode)
        {
            List<AdminBakingFullInforDTO> backingList = new List<AdminBakingFullInforDTO>();
            // Check authen.
            if (User.Identity == null || !User.Identity.IsAuthenticated)
            ...
            try
            {
                backingList = UserRepository.Instance.GetBackingForCreatedProjectExport(projectcode, User.Identity.Name);
            }
            catch (KeyNotFoundException)
            {
                return
