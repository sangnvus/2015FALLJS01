...
!function($) {
	...
	var DateRangePicker = function(element, options, cb) {
		...
		if (!hasOptions
				|| (typeof options.startDate == 'undefined' && typeof options.endDate == 'undefined')) {
	...
	DateRangePicker.prototype = {
		...
		move : function() {
			...
			if ($(this.container).find('.calendar').is(':visible')) {
				var padding = 24; // FIXME: this works for the default
				// styling, but isn't flexible
				minWidth += $(this.container).find('.calendar').outerWidth()
						* 2 + padding;
			}
			if (this.opens == 'left') {
				this.container.css({
			...
			} else if (this.opens == 'left_top') {
				this.container.css({
					top : this.element.offset().top - this.container.height(),
					right : $(window).width() - this.element.offset().left
							- this.element.outerWidth(),
					left : 'auto',
					'min-width' : minWidth
			...
			for ( var range in this.ranges) {
				if (this.timePicker) {
					if (this.startDate.isSame(this.ranges[range][0])
						...
						customRange = false;
						this.container.find('.ranges li:eq(' + i + ')')
								.addClass('active');
					}
				} else {
		...
		renderCalendar : function(calendar, selected, minDate, maxDate) {
			...
			for ( var row = 0; row < 6; row++) {
				...
				for ( var col = 0; col < 7; col++) {
					var cname = 'available ';
					cname += (calendar[row][col].month() == calendar[1][1]
							.month()) ? '' : 'off';

					if ((minDate && calendar[row][col].isBefore(minDate))
							|| (maxDate && calendar[row][col].isAfter(maxDate))) {
						cname = ' off disabled ';
					} else if (calendar[row][col].format('YYYY-MM-DD') == selected
							.format('YYYY-MM-DD')) {
						cname += ' active ';
						...
						if (calendar[row][col].format('YYYY-MM-DD') == this.endDate
								.format('YYYY-MM-DD')) {
							cname += ' end-date ';
						}
